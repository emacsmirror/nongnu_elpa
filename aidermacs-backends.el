...
1. False positives: Any text containing \"(Y)es/(N)o\" will be treated
   as a prompt, potentially triggering notifications prematurely.  This
   includes:
...
2. False positive probability: Moderate.  The pattern is specific
   enough to avoid most casual text, but technical discussions about
   aider's prompts may trigger it.
   In normal usage, false positives are infrequent but possible.
3. No context awareness: The regexp doesn't check if the pattern
   appears at the end of a line (where prompts usually are) or is
   followed by \" [Yes]:\".
   This increases recall but reduces precision.
4. Timing implications: Premature detection may cause notifications to
   be sent before the user has actually seen the prompt, reducing
   their usefulness.")
