  ...
  outputs =
    ...
    inputs.flake-parts.lib.mkFlake { inherit inputs; } (
      { lib, ... }:
      {
        imports =
          let
            isWanted = file: isNix file && !isSpecialNix file && !isIgnored file;
          ...
          in
          lib.fileset.toList (lib.fileset.fileFilter isWanted ./.);

        systems = [
          "x86_64-linux"
          "aarch64-linux"
          # "aarch64-darwin" # ci of this system is too slow
        ];
      }
    );
}
