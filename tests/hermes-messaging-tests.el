...
(ert-deftest hermes-messaging-client-acquisition-failure-clears-mutation-lock ()
  "Synchronous acquisition failures report and cannot strand the mutation lock."
  (let (messages)
    (cl-letf (((symbol-function 'hermes-browser--with-client)
      ...
      (with-temp-buffer
        ...
        (hermes-messaging--revert)
        (should (equal messages '("Hermes: start failed" "Hermes: start failed")))))))
