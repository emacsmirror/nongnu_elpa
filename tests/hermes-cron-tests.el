  ...
  (let ((first (hermes--promise-make))
    ...
    (cl-letf (((symbol-function 'hermes-browser--run-on-client)
              ...
              ((symbol-function 'hermes-cron--display-detail)
               (lambda (job _runs &optional _instance)
  ...
  (let ((first (hermes--promise-make))
    ...
    (cl-letf (((symbol-function 'hermes-browser--run-on-client)
              ...
              ((symbol-function 'hermes-cron--display-run)
               (lambda (_id messages &optional _profile _instance)
...
(ert-deftest hermes-cron-view-claims-instance-before-render-and-display ()
  "Details and runs retain their exact owner, even beside foreign namesakes."
  (let* ((instance '("cron-owner" . "http://cron.invalid"))
         (hermes-instances (list instance))
         (job (generate-new-buffer "*Hermes Cron Job*"))
         (run (generate-new-buffer "*Hermes Cron Run*"))
         targets)
    (unwind-protect
        (progn
          (dolist (buffer (list job run))
            (with-current-buffer buffer (insert "foreign notes") (special-mode)))
          (cl-letf (((symbol-function 'hermes-browser--with-client)
                     (lambda (fn) (funcall fn 'client #'ignore)))
                    ((symbol-function 'hermes-cron--fetch-job)
                     (lambda (&rest _) (hermes--promise-resolved nil)))
                    ((symbol-function 'hermes-cron--fetch-runs)
                     (lambda (&rest _)
                       (hermes--promise-resolved '((runs . (((id . "run"))))))))
                    ((symbol-function 'hermes-cron--fetch-run-messages)
                     (lambda (&rest _)
                       (hermes--promise-resolved '((messages . (((content . "literal"))))))))
                    ((symbol-function 'hermes-cron--format-job)
                     (lambda (_job)
                       (should (eq major-mode 'hermes-cron-detail-mode))
                       (should (equal hermes-instance instance))
                       "job"))
                    ((symbol-function 'hermes-cron--format-message)
                     (lambda (_message)
                       (should (eq major-mode 'hermes-cron-run-mode))
                       (should (equal hermes-instance instance))
                       "transcript"))
                    ((symbol-function 'pop-to-buffer)
                     (lambda (buffer &rest _)
                       ...
                       (with-current-buffer buffer
                         (should (hermes-buffer--owned-p))
                         (should (equal hermes-instance instance))))))
            (hermes-test-with-cron-buffer (list (hermes-test--cron-entry))
              (setq hermes-instance instance)
              (hermes-cron-show))
            (should (= (length targets) 1))
            (with-current-buffer (car targets)
              (goto-char (point-min))
              (goto-char (prop-match-beginning
                          (text-property-search-forward 'hermes-cron-run-id "run" #'equal)))
              (call-interactively #'hermes-cron-show-run-log)))
          (should (= (length targets) 2))
          (dolist (buffer (list job run))
            (with-current-buffer buffer
              (should (equal (buffer-string) "foreign notes"))
              (should-not hermes-instance))))
      (mapc #'kill-buffer (append targets (list job run))))))

(ert-deftest hermes-cron-existing-list-lazily-arms-one-refresh-timer ()
  "Refreshing an existing list starts one timer and disabling retires it."
  (let* ((hermes-cron-auto-refresh-interval nil)
         (buffer (hermes-buffer--get " *cron lazy refresh*" #'hermes-cron-mode))
         (starts 0) cancelled tick)
    (unwind-protect
        (with-current-buffer buffer
          (setq hermes-instance '("cron" . "http://cron.invalid"))
          (cl-letf (((symbol-function 'hermes-browser--with-client)
                     (lambda (fn) (funcall fn 'client #'ignore)))
                    ((symbol-function 'hermes-cron--api)
                     (lambda (&rest _) (hermes--promise-resolved '((jobs . nil)))))
                    ((symbol-function 'run-at-time)
                     (lambda (_delay _repeat fn &rest _)
                       (cl-incf starts) (setq tick fn) 'owned-timer))
                    ((symbol-function 'cancel-timer)
                     (lambda (timer) (push timer cancelled))))
            (setq hermes-cron-auto-refresh-interval 5)
            (revert-buffer nil t)
            (revert-buffer nil t)
            (should (= starts 1))
            (should (eq hermes-cron--auto-refresh-timer 'owned-timer))
            (setq hermes-cron-auto-refresh-interval nil)
            (funcall tick)
            (should-not hermes-cron--auto-refresh-timer)
            (should (equal cancelled '(owned-timer)))
            (funcall tick)
            (should (= starts 1))))
      (kill-buffer buffer))))
