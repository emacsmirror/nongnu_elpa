  ...
  (let ((fetch (hermes--promise-make)) prompted updated)
    (cl-letf (((symbol-function 'hermes-browser--with-client)
              ...
              ((symbol-function 'hermes-cron--update-job)
               (lambda (&rest _) (setq updated t) (hermes--promise-resolved nil)))
              ((symbol-function 'message) #'ignore))
      (hermes-test-with-cron-buffer (list (hermes-test--cron-entry))
        (hermes-browser--own-instance '("a" . "http://a"))
        (hermes-cron-edit)
        (hermes-browser--own-instance '("b" . "http://b"))
        (hermes--promise-resolve fetch (hermes-test--cron-entry))))
    (should-not prompted)
    (should-not updated)))
    ...
    (cl-letf (((symbol-function 'hermes-browser--with-client)
              ...
              ((symbol-function 'hermes-cron--read-updates)
               (lambda (&rest _)
                 (setq prompted t)
                 (hermes-browser--next-request-generation)
                 '((name . "edited") (schedule . "*/5 * * * *")
                   (prompt . "new") (deliver . "local") (skills))))
              ((symbol-function 'hermes-cron--update-job)
               (lambda (&rest _)
                 (setq updated t)
      ...
      (hermes-test-with-cron-buffer (list (hermes-test--cron-entry))
        ...
        (hermes--promise-resolve
         fetch '((id . "j1") (profile . "work") (name . "nightly")
                 (schedule . ((expr . "0 0 * * *"))) (prompt . "old")
                 (deliver . "local") (skills . ("old-skill"))))))
    (should prompted)
    (should-not updated)
...
(ert-deftest hermes-cron-prompt-cancel-does-not-mutate ()
  "Cancelling either prompt editor performs no cron mutation."
  (let (calls create-cancelled refreshed)
    (cl-letf (((symbol-function 'read-string)
               (lambda (&rest _) "value"))
              ((symbol-function 'read-string-from-buffer)
              ...
              ((symbol-function 'hermes-browser--with-client)
               (lambda (fn) (funcall fn 'fake-client #'ignore)))
              ((symbol-function 'hermes-cron--api)
               (lambda (_client method _path &optional _body _query)
                 (push method calls)
                 (hermes--promise-resolved
      ...
      (condition-case nil
          (call-interactively #'hermes-cron-create)
        (quit (setq create-cancelled t)))
      (hermes-test-with-cron-buffer (list (hermes-test--cron-entry))
        (hermes-cron-edit)))
    (should create-cancelled)
