...
(defmacro hermes-mcp-test--transport-operation (&rest body)
  "Run BODY through the real REST builder with an isolated typed client."
  (declare (indent 0))
  `(let ((client (make-hermes-dashboard-transport-client
                  :base-url "https://backend.example" :token "original-token"))
         (hermes-dashboard-transport-url "https://backend.example")
         (hermes-instances '(("test" . "https://backend.example")))
         requests timers cancelled opened messages
         (released 0) (refreshed 0) (auth-calls 0))
     (cl-letf (((symbol-function 'hermes-browser--existing-client) #'ignore)
               ...
               ((symbol-function 'hermes-dashboard-transport-release)
                (lambda (owner) (should (eq owner client)) (cl-incf released)))
               ((symbol-function 'hermes-dashboard-transport--http-json-request-async)
                (lambda (request)
                  (let ((promise (hermes--promise-make)))
                    (push (cons request promise) requests)
                    promise)))
               ((symbol-function 'hermes-dashboard-transport-api-auth-async)
                (lambda (&rest _)
                  (cl-incf auth-calls)
               ...
               ((symbol-function 'run-at-time)
                (lambda (&rest args) (push args timers) args))
       ...
       (with-temp-buffer
         ...
         (puthash "srv" '((name . "srv") (url . "https://mcp.example") (auth . "oauth"))
                  hermes-mcp--servers)
         (setq tabulated-list-entries '(("srv" ["srv" "http" "on" "configured" "0"])))
         (tabulated-list-print)
         (goto-char (point-min))
         ,@body))))

(ert-deftest hermes-mcp-transport-retirement-retires-late-oauth-start ()
  "Retiring before the HTTP receipt releases ownership without browser work."
  (dolist (retirement '(stop replace))
    (hermes-mcp-test--transport-operation
      (hermes-mcp-authenticate)
      (let ((start (car requests)))
        (should (equal (plist-get (car start) :method) "POST"))
        (if (eq retirement 'stop)
            (hermes-dashboard-transport-stop client)
          (hermes-dashboard-transport--reconnect-effects
           (hermes-dashboard-transport--take-reconnect client "Replace" nil)
           nil (lambda () t)))
        (should-not hermes-mcp--operation)
        (should (= released 1))
        (should (= (length cancelled) 1))
        (hermes--promise-resolve
         (cdr start) '(:body ((flow_id . "late-flow") (server_name . "srv")
...
(ert-deftest hermes-mcp-transport-current-monitoring-completes ()
  "The unchanged transport still opens OAuth, polls, and completes once."
  (dolist (kind '(oauth install))
    (hermes-mcp-test--transport-operation
      (if (eq kind 'oauth) (hermes-mcp-authenticate)
        (hermes-mcp--start-operation kind "srv" nil))
      (hermes--promise-resolve
       (cdar requests)
       (list :body (if (eq kind 'oauth)
                       '((flow_id . "flow") (server_name . "srv")
                         (status . "authorization_required")
                         (authorization_url . "https://auth.example/authorize"))
                     '((ok . t) (background . t) (action . "mcp-install-srv-123")))))
      (should (= (length opened) (if (eq kind 'oauth) 1 0)))
      (should (= released 0))
      (hermes-mcp--poll hermes-mcp--operation)
      (should (equal (plist-get (caar requests) :method) "GET"))
      (hermes--promise-resolve
       (cdar requests)
       (list :body (if (eq kind 'oauth)
                       '((flow_id . "flow") (server_name . "srv") (status . "approved"))
...
(ert-deftest hermes-mcp-transport-retirement-releases-monitoring ()
  "Retirement cancels OAuth/install timers and leaves a usable successor."
  (dolist (kind '(oauth install))
    (dolist (retirement '(stop replace))
      (hermes-mcp-test--transport-operation
        (if (eq kind 'oauth) (hermes-mcp-authenticate)
        ...
        (hermes--promise-resolve
         (cdar requests)
         (list :body (if (eq kind 'oauth)
                         '((flow_id . "flow") (server_name . "srv") (status . "starting"))
                       '((ok . t) (background . t) (action . "mcp-install-srv-123")))))
        (let ((old hermes-mcp--operation))
          (hermes-mcp--poll old)
          (let ((poll (car requests)))
            (if (eq retirement 'stop)
                (hermes-dashboard-transport-stop client)
              ;; Use the real replacement transition, without dialing a socket.
              (hermes-dashboard-transport--reconnect-effects
               (hermes-dashboard-transport--take-reconnect client "Replace" nil)
               nil (lambda () t)))
            (should-not hermes-mcp--operation)
            ...
            (setq client (make-hermes-dashboard-transport-client
                          :base-url "https://backend.example" :token "successor-token"))
            (hermes-mcp-authenticate)
            (let ((successor hermes-mcp--operation)
                  (before requests)
                  (before-timers timers))
              (hermes--promise-reject (cdr poll) "Late old failure")
              (hermes-mcp--poll old)
              (should (eq successor hermes-mcp--operation))
              (should (eq before requests))
              (should (eq before-timers timers))
              (should-not messages)
              (should (= released 1))
              (should (= auth-calls 0))
              (hermes-mcp-cancel)
...
(ert-deftest hermes-mcp-late-cancel-cleanup-keeps-original-destination ()
  "Cancelled late starts delete only on an unchanged original transport."
  (dolist (replacement '(nil generation endpoint))
    (hermes-mcp-test--transport-operation
      (hermes-mcp-authenticate)
      (let ((start (car requests)))
        (hermes-mcp-cancel)
        (pcase replacement
          ('generation (cl-incf (hermes-dashboard-transport-client-generation client)))
          ('endpoint (setf (hermes-dashboard-transport-client-base-url client)
        ...
        (hermes--promise-resolve
         (cdr start) '(:body ((flow_id . "late-flow") (server_name . "srv")
                             (status . "authorization_required")
                             (authorization_url . "https://auth.example/authorize"))))
        (should (= (length requests) (if replacement 1 2)))
        (unless replacement
          (should (equal (plist-get (caar requests) :method) "DELETE"))
          (should (equal (plist-get (caar requests) :url)
                         "https://backend.example/api/mcp/oauth/flows/late-flow")))
        (should-not opened)
        (should (= auth-calls 0))
        (should (= released 1))))))

(ert-deftest hermes-mcp-late-cancel-auth-wait-cannot-cross-retirement ()
  "Late-flow cleanup must retain its destination through an auth wait too."
    ...
    (hermes-mcp-test--transport-operation
      ...
      (let ((start (car requests))
        ...
        (cl-letf (((symbol-function 'hermes-dashboard-transport-api-auth-async)
                   (lambda (&rest _) auth)))
          (hermes--promise-resolve (cdr start) '(:body ((flow_id . "late-flow"))))
          (should (= (length requests) 1))
          (when retired (hermes-dashboard-transport-stop client))
          (hermes--promise-resolve auth '(:base-url "https://backend.example"))
          (should (= (length requests) (if retired 1 2)))
          (should (= released 1))
          (should-not opened))))))

(ert-deftest hermes-mcp-poll-auth-wait-retains-owner ()
  "A poll waiting for authentication cannot dispatch after transport retirement."
  (dolist (kind '(oauth install))
    (hermes-mcp-test--transport-operation
      (hermes-mcp--start-operation kind "srv" nil)
      (hermes--promise-resolve
...
(ert-deftest hermes-mcp-real-acquisition-monitoring-completes ()
  "Default and named public entries survive acquisition and initial auth waits."
  (dolist (named '(nil t))
    (dolist (kind '(oauth install))
        ...
        (hermes-mcp-test--transport-operation
          (let* ((hermes-instances (and named hermes-instances))
            ...
            (setq-local hermes-instance
                        (if named (car hermes-instances)
                          (cons "default" "https://backend.example")))
            (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "srv"))
              ...
              (when delayed
                (should-not requests)
                (setf (hermes-dashboard-transport-client-token client) "test-token")
                (hermes--promise-resolve auth '(:base-url "https://backend.example")))
              (when (eq kind 'install)
                ...
                (hermes--promise-resolve
                 (cdar requests)
                 '(:body ((entries . (((name . "srv") (install_url) (bootstrap)))))))
                (when delayed
                  (should (= (length requests) 1))
                  (setf (hermes-dashboard-transport-client-token client) "test-token")
                  (hermes--promise-resolve auth '(:base-url "https://backend.example"))))
              (should (equal (plist-get (caar requests) :method) "POST"))
              (should (equal (plist-get (caar requests) :url)
                             (concat "https://backend.example/api/mcp"
                                     (if (eq kind 'oauth) "/servers/srv/auth"
                                       "/catalog/install"))))
              (let ((operation hermes-mcp--operation))
                (should (hermes-mcp--operation-current-p operation))
                (hermes--promise-resolve
                 (cdar requests)
                 (list :body (if (eq kind 'oauth)
                                 '((flow_id . "flow") (server_name . "srv")
                               ...
                               '((ok . t) (background . t) (action . "mcp-install-srv-123")))))
                (should (= (length opened) (if (eq kind 'oauth) 1 0)))
                (should (eq operation hermes-mcp--operation))
                ;; Deliver the scheduled poll, not merely a helper-only read.
                (apply (nth 2 (car timers)) (nthcdr 3 (car timers)))
                (should (equal (plist-get (caar requests) :method) "GET"))
                (hermes--promise-resolve
                 (cdar requests)
                 (list :body (if (eq kind 'oauth)
                ...
                (should-not hermes-mcp--operation)
                (should (= refreshed 1))
                (should (= released (if (eq kind 'oauth) 1 2)))))))))))

(ert-deftest hermes-mcp-real-default-install-retry-after-failure ()
  "Failed default monitoring releases its owner and a fresh install completes."
  (hermes-mcp-test--transport-operation
    (let ((hermes-instances nil))
      (setq-local hermes-instance (cons "default" "https://backend.example"))
      (hermes-mcp--start-operation 'install "srv" nil)
      (hermes--promise-resolve
       (cdar requests) '(:body ((ok . t) (background . t) (action . "mcp-install-srv-123"))))
      (let ((old hermes-mcp--operation))
        (should old)
        (hermes-mcp--poll old)
        (hermes--promise-reject (cdar requests) "Monitor failed")
        (should-not hermes-mcp--operation)
        (should (= released 1))
        (hermes-mcp--start-operation 'install "srv" nil)
        (let ((next hermes-mcp--operation) (before requests))
          (hermes-mcp--poll old)
          (should (eq before requests))
          (should (eq next hermes-mcp--operation))
          (hermes--promise-resolve
           (cdar requests) '(:body ((ok . t) (background . t) (action . "mcp-install-srv-456"))))
          (hermes-mcp--poll next)
          (should (equal (plist-get (caar requests) :url)
                         "https://backend.example/api/actions/mcp-install-srv-456/status?lines=1"))
          (hermes--promise-resolve (cdar requests) '(:body ((running . :false) (exit_code . 0))))
          (should-not hermes-mcp--operation)
          (should (= released 2))
          (should (= refreshed 1)))))))

(ert-deftest hermes-mcp-real-acquisition-unowned-and-retired-origin ()
  "An unowned origin works; retirement during lookup prevents dispatch."
  (dolist (replace '(nil t))
    (hermes-mcp-test--transport-operation
      (let ((hermes-instances nil))
        (setq-local hermes-instance nil)
        (cl-letf (((symbol-function 'hermes-browser--existing-client)
          ...
          (hermes-mcp-authenticate)
          (if replace
              (progn (should-not requests) (should (= released 1)))
            (should (= (length requests) 1))
            (hermes--promise-resolve
             (cdar requests) '(:body ((flow_id . "flow") (server_name . "srv") (status . "approved"))))
            (should (= refreshed 1))
            (should (= released 1))))))))

(ert-deftest hermes-mcp-real-default-auth-wait-rejects-successor ()
  "A real default acquisition cannot lend a pending write to a successor."
  (dolist (retire '(identity endpoint generation))
    (hermes-mcp-test--transport-operation
      (let ((hermes-instances nil) (auth (hermes--promise-make)))
        (setq-local hermes-instance (cons "default" "https://backend.example"))
        (setf (hermes-dashboard-transport-client-token client) nil)
        (cl-letf (((symbol-function 'hermes-dashboard-transport-api-auth-async)
                   (lambda (&rest _) auth)))
          ...
          (should-not requests)
          (pcase retire
            ('identity (setq-local hermes-instance (copy-tree hermes-instance)))
            ('endpoint (setf (hermes-dashboard-transport-client-base-url client)
                             "https://successor.example"))
            ('generation (cl-incf (hermes-dashboard-transport-client-generation client))))
          (hermes--promise-resolve auth '(:base-url "https://backend.example"))
...
(ert-deftest hermes-mcp-install-background-is-not-completion ()
  (hermes-mcp-test--operation
    ...
    (let ((operation hermes-mcp--operation) status-request)
      (should (equal (json-parse-string (json-serialize (nth 2 (car requests)))
                                       :object-type 'alist)
...
(ert-deftest hermes-mcp-stdio-env-body-serialization ()
  "Stdio environment names and literal values survive JSON encoding."
  (dolist (env '(nil (("MCP_TEST_VALUE" . "synthetic=α \"quoted\"")
                     ("MCP_TEST_EMPTY" . ""))))
    (let* ((body (hermes-mcp--create-body
                  "new" "stdio" "node" '("server.js") env "none" nil))
           (decoded (json-parse-string (json-serialize body)))
           (object (gethash "env" decoded)))
      (should (hash-table-p object))
      (should (= (hash-table-count object) (length env)))
      (dolist (pair env)
        (should (equal (gethash (car pair) object) (cdr pair)))))))

(ert-deftest hermes-mcp-add-stdio-env-serialized-request ()
  "Public Add sends reader values through the actual JSON request builder."
  (let ((executor (symbol-function 'hermes-dashboard-transport--http-json-request-async)))
    (hermes-mcp-test--transport-operation
      (let* ((texts '("new" "node" "server.js" "" "MCP_TEST_VALUE"
                      "MCP_TEST_EMPTY" "MCP_TEST_VALUE" ""))
             (values '("replaced" "" "synthetic=α \"quoted\""))
             wire
             (hermes-dashboard-transport-http-request-async-function
              (lambda (url &rest keys)
                (setq wire (cons url keys))
                (hermes--promise-resolved '(:body ((ok . t)))))))
        (cl-letf (((symbol-function 'hermes-dashboard-transport--http-json-request-async)
                  ...
                  ((symbol-function 'read-passwd) (lambda (&rest _) (pop values)))
                  ((symbol-function 'completing-read) (lambda (&rest _) "stdio")))
          (hermes-mcp-add)
          (should (equal (car wire) "https://backend.example/api/mcp/servers"))
          (should (equal (plist-get (cdr wire) :method) "POST"))
          (let* ((body (json-parse-string (plist-get (cdr wire) :data)))
                 (env (gethash "env" body)))
            (should (equal (gethash "name" body) "new"))
            (should (equal (gethash "command" body) "node"))
            (should (equal (gethash "args" body) ["server.js"]))
            (should (= (hash-table-count env) 2))
            (should (equal (gethash "MCP_TEST_VALUE" env) "synthetic=α \"quoted\""))
            (should (equal (gethash "MCP_TEST_EMPTY" env) "")))
          (should (member "synthetic=α \"quoted\"" (plist-get (cdr wire) :secrets)))
          (should-not texts)
          (should-not values)
          (should (= refreshed 1))
          (should (= released 1)))))))

(ert-deftest hermes-mcp-catalog-env-serialized-request ()
  "Public Catalog serializes declared credentials and completes installation."
  (let ((executor (symbol-function 'hermes-dashboard-transport--http-json-request-async)))
    (hermes-mcp-test--transport-operation
      (let* ((values '("synthetic=α \"quoted\"" ""))
             wire
             (hermes-dashboard-transport-http-request-async-function
              (lambda (url &rest keys)
                (push (cons url keys) wire)
                (hermes--promise-resolved
                 (if (equal (plist-get keys :method) "GET")
                     '(:body ((entries . (((name . "catalog") (install_url) (bootstrap)
                                           (required_env . (((name . "MCP_TEST_VALUE") (required . t))
                                                            ((name . "MCP_TEST_OPTIONAL")))))))))
                   '(:body ((ok . t) (background . :false))))))))
        (cl-letf (((symbol-function 'hermes-dashboard-transport--http-json-request-async)
                   executor)
                  ((symbol-function 'read-passwd) (lambda (&rest _) (pop values)))
          ...
          (should (equal (caar wire) "https://backend.example/api/mcp/catalog/install"))
          (should (equal (plist-get (cdar wire) :method) "POST"))
          (let* ((body (json-parse-string (plist-get (cdar wire) :data)))
                 (env (gethash "env" body)))
            (should (equal (gethash "name" body) "catalog"))
            (should (eq (gethash "enable" body) t))
            (should (= (hash-table-count env) 1))
            (should (equal (gethash "MCP_TEST_VALUE" env) "synthetic=α \"quoted\"")))
          (should (member "synthetic=α \"quoted\"" (plist-get (cdar wire) :secrets)))
          (should-not values)
          (should-not hermes-mcp--operation)
          (should (= refreshed 1))
          (should (= released 2)))))))
...
(ert-deftest hermes-mcp-catalog-nonbootstrap-keeps-dashboard-scope ()
  ...
  (hermes-mcp-test--operation
    (cl-letf (((symbol-function 'completing-read) (lambda (&rest _) "catalog"))
      ...
      (should (equal (cl-subseq (car requests) 0 2) '("POST" "/catalog/install")))
      (should (equal (json-parse-string (json-serialize (nth 2 (car requests)))
                                       :object-type 'alist)
                     '((name . "catalog") (enable . t) (env . ((API_KEY . "short-secret"))))))
      (should (eq (plist-get (nth 4 (car requests)) :client) 'exact-client))
