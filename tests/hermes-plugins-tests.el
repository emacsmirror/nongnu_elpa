...
(ert-deftest hermes-plugins-mutation-acquisition-settles-and-retries ()
      ...
      (with-temp-buffer
        ...
        (let ((hermes-instances (list hermes-instance))
          ...
          (cl-letf (((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
                     ...
                     (lambda (&rest _)
                       ...
                       (if (> attempts 1) client
                         (when replace
                           ...
                           (setq hermes-plugins--busy successor))
            ...
            (should (eq condition
                        (condition-case err
                            (progn (hermes-plugins--request "DELETE" "/api/dashboard/agent-plugins/test" nil t) nil)
                          ((error quit) (car err)))))
            (if replace
                (should (eq hermes-plugins--busy successor))
              (should-not hermes-plugins--busy)
              (hermes-plugins--request "DELETE" "/api/dashboard/agent-plugins/test" nil t)
              (should (= attempts 2))
              (should (= released 1))
              (should-not hermes-plugins--busy))))))))

(ert-deftest hermes-plugins-mutation-delayed-auth-owner-fence ()
          ...
          (cl-letf (((symbol-function 'hermes-browser--with-client)
            ...
            (when (buffer-live-p buffer)
              ...
              (when (derived-mode-p 'hermes-plugins-mode)
                ...
                (cond
                 ((memq ending '(success failure semantic))
                  (should-not hermes-plugins--busy)
                  (should (string-match-p "failed" hermes-plugins--status)))
                 ((memq ending '(refresh reconnect endpoint))
                  (should-not hermes-plugins--busy)
                  (should (string-match-p "Interrupted.*stale" hermes-plugins--status)))
                 (t (should (equal hermes-plugins--status
                                   "Updating; awaiting readback"))))))))))))
...
(ert-deftest hermes-plugins-refresh-failure-retains-inert-snapshot-and-retries ()
  "Public refresh keeps rows on both failure paths without authorizing writes."
  (dolist (failure '(acquisition rejection))
    (with-temp-buffer
      (hermes-plugins-mode)
      (hermes-buffer--claim 'hermes-plugins-mode)
      (setq hermes-instance '("test" . "http://example.invalid"))
      (let* ((hub '(:plugins [(:name "retained")] :providers nil :hermes_home nil))
             (pending (hermes--promise-make))
             (fail (eq failure 'acquisition)) (releases 0))
        (hermes-plugins--render hub "Ready")
        (goto-char (point-min))
        (let ((text (buffer-string)))
          (cl-letf (((symbol-function 'hermes-browser--existing-client) #'ignore)
                    ((symbol-function 'hermes-dashboard-transport-acquire)
                     (lambda (&rest _)
                       (when fail (error "Unavailable"))
                       (make-hermes-dashboard-transport-client
                        :base-url "http://example.invalid")))
                    ((symbol-function 'hermes-dashboard-transport-release)
                     (lambda (_) (cl-incf releases)))
                    ((symbol-function 'hermes-plugins--api)
                     (lambda (_client _method path _body)
                       (if (equal path "/api/status") (hermes--promise-resolved nil)
                         pending))))
            (if fail (should-error (call-interactively #'hermes-plugins-refresh))
              (call-interactively #'hermes-plugins-refresh)
              (hermes--promise-reject pending "secret failure body"))
            (should (equal hermes-plugins--snapshot hub))
            (should (equal (buffer-string) text))
            (should (string-match-p "stale" hermes-plugins--status))
            (should-not (string-match-p "secret" hermes-plugins--status))
            (should-error (hermes-plugins-enable) :type 'user-error)
            (should-error (hermes-plugins-install) :type 'user-error)
            (setq fail nil pending (hermes--promise-make))
            (call-interactively #'hermes-plugins-refresh)
            (hermes--promise-resolve pending hub)
            (should-not hermes-plugins--snapshot-stale-p)
            (should (equal (hermes-transport--get (hermes-plugins--selected) 'name)
                           "retained"))
            (should (= releases (if (eq failure 'acquisition) 1 2)))))))))

(ert-deftest hermes-plugins-render-failure-retains-last-accepted-inventory ()
  "Refresh and mutation readback publish only completely rendered inventories."
  (dolist (command '(hermes-plugins-refresh hermes-plugins-enable))
    (dolist (fault '(entries print-error print-quit))
      (with-temp-buffer
        (hermes-plugins-mode)
        (hermes-buffer--claim 'hermes-plugins-mode)
        (setq hermes-instance '("test" . "http://example.invalid"))
        (let* ((hub '(:plugins [(:name "retained" :runtime_status "inactive")]
                      :providers nil :hermes_home nil))
               (replacement `(:plugins [(:name "replacement" :runtime_status
                                         ,(if (eq fault 'entries) 1 "enabled"))]))
          ...
          (let ((text (buffer-string)) (entries tabulated-list-entries))
            (cl-letf (((symbol-function 'hermes-browser--existing-client) #'ignore)
                      ...
                      ((symbol-function 'yes-or-no-p) (lambda (&rest _) t))
                      ((symbol-function 'hermes-plugins--api)
                       (lambda (_client method path _body)
                         (cl-incf calls)
                         (cond ((equal path "/api/status") (hermes--promise-resolved nil))
                               ((equal method "POST") (hermes--promise-resolved '(:ok t)))
                               (t pending)))))
              (call-interactively command)
              (let ((tabulated-list-printer
                     (if (eq fault 'entries) printer
                       (lambda (id columns)
                         (funcall printer id columns)
                         (signal (if (eq fault 'print-quit) 'quit 'error)
                                 '("Private printer failure"))))))
                (hermes--promise-resolve pending replacement))
              (should (eq hermes-plugins--snapshot hub))
              (should (eq tabulated-list-entries entries))
              (should (equal-including-properties text (buffer-string)))
              (should hermes-plugins--snapshot-stale-p)
              (should-not hermes-plugins--busy)
              (should (string-match-p "stale" hermes-plugins--status))
              (should-not (string-match-p "Private" hermes-plugins--status))
              (should (= releases 1))
              (let ((before calls) (readers 0))
                (cl-letf (((symbol-function 'yes-or-no-p)
                           (lambda (&rest _) (cl-incf readers) nil))
                          ((symbol-function 'read-string)
                           (lambda (&rest _) (cl-incf readers) ""))
                          ((symbol-function 'completing-read)
                           (lambda (&rest _) (cl-incf readers) "compressor")))
                  (dolist (action '(hermes-plugins-enable hermes-plugins-install
                                    hermes-plugins-select-context-engine))
                    (should-error (call-interactively action) :type 'user-error)))
                (should (zerop readers))
                (should (= before calls)))
...
(ert-deftest hermes-plugins-stop-settles-exact-owner-before-late-http ()
  "Transport stop releases the operation without admitting late publications."
  (dolist (reject '(nil t))
    (dolist (successor '(nil t))
      (with-temp-buffer
        ...
        (hermes-buffer--claim 'hermes-plugins-mode)
        (setq hermes-instance '("test" . "http://example.invalid"))
        (let* ((hub '(:plugins [(:name "retained")]))
               (client (make-hermes-dashboard-transport-client
                        :base-url "http://example.invalid"))
               (pending (hermes--promise-make))
               (next (hermes--promise-make))
               (releases 0) (calls 0))
          (hermes-plugins--render hub "Ready")
          (goto-char (point-min))
          (let ((text (buffer-string)))
            (cl-letf (((symbol-function 'hermes-browser--existing-client) #'ignore)
                      ...
                      ((symbol-function 'hermes-plugins--api)
                       (lambda (&rest _) (cl-incf calls) pending)))
              (call-interactively #'hermes-plugins-enable)
              ...
              (when successor
                (cl-letf (((symbol-function 'hermes-plugins--api)
                           (lambda (&rest _) (cl-incf calls) next)))
                  (call-interactively #'hermes-plugins-refresh)))
              (let ((status hermes-plugins--status) (before calls))
                (if reject (hermes--promise-reject pending "Private HTTP failure")
                  (hermes--promise-resolve pending '(:ok t)))
                (should (equal hermes-plugins--status status))
                (should (eq hermes-plugins--snapshot hub))
                (should (equal text (buffer-string)))
                (should (= calls before))
                (should (= releases 1)))
              (when successor
                (hermes--promise-reject next "Private retry failure")
