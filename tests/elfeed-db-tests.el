...
(ert-deftest elfeed-db-tag ()
  (with-elfeed-test
   (let* ((feed (elfeed-test-generate-feed))
     ...
     (should (equal (list entry) (apply #'elfeed-tag entry tags)))
     (should-not (apply #'elfeed-tag entry tags))
     (should (equal (list entry) (elfeed-untag entry 'unread)))
     (should-not (elfeed-untag entry 'unread))
