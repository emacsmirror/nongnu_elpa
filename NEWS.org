...
- =elfeed-db-save-idle=: New customization option to delay database saving.
- =elfeed-use-libxml=: Enable by default if libxml is available.
- =elfeed-default-directory=: Default to user home.

** Search

- =elfeed-search-tag=, =elfeed-search-untag=, =elfeed-search-tag-unread=,
  =elfeed-search-untag-unread=, =elfeed-search-toggle=: Shorter command names.
  Improve doc strings.
- =elfeed-search-confirm-tag=: New customization variable to confirm tagging
  multiple entries.
- =elfeed-search-max-entries= and =elfeed-search-extend:= New customization variable
  and new command to extend truncated search list.
...
- =elfeed-tree-icon-style=: Style to use for the tree icons (=ascii=, =box=, =nerd=).
- =elfeed-tree-icon-presets=: Customizable variable defining icon style presets.
- =elfeed-tree-and-search=: New entry point command to start Elfeed with tree and
  search windows side by side.
