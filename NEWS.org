...
- =elfeed-search-last-update-relative=: New option to show last database update
...
- =elfeed-show-fetch-link=: New command to fetch link content and insert it into
  ...
  content.
#+begin_src emacs-lisp
;; Automatically fetch the link content
(setq elfeed-feeds
...
- =elfeed-show-readable=: Command to toggle readable mode. Readable mode can be
  enabled automatically via =:readable= and even combined with =:fetch-link=.
#+begin_src emacs-lisp
...
- Show error count in header line. Clicking on the error count will show the log
  buffer.
