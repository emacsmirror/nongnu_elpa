...
* NEWS
** Version 4.0.8
*** New user var helm-buffers-details
which allow customizing which columns are displayed in
`helm-source-buffers-list`.

*** Helm buffer is now read-only
So it is now impossible to edit it inadvertantly.

*** It is now possible to copy a set of files to different directories
`helm-ff-mcp` has been rewrited, previously it was only possible to
copy a single file to several directories.

** Version 4.0.7
*** Improve helm-dwim-target-directory
You can use now M-n to cycle in default directories bound to each
window.
