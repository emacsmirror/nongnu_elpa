...
+ The bridge can evaluate Elisp, edit buffers, and run shell commands. It
  only accepts loopback bind addresses.  These dangerous tools are hidden by
  default; enable them explicitly with
...
| =codex-ide-mcp-dangerous-tools-enabled= | Expose eval/edit/shell tools |
...
MCP enable-dangerous-tools-host-port/context-auto-start).
