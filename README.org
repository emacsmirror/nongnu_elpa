...
#+language: en
#+export_file_name: elfeed.texi
#+texinfo_dir_category: Emacs misc features
#+texinfo_dir_title: Elfeed: (elfeed).
#+texinfo_dir_desc: Atom/RSS feed reader

#+html: <a href="https://www.gnu.org/software/emacs/"><img alt="GNU Emacs" src="https://github.com/minad/corfu/blob/screenshots/emacs.svg?raw=true"/></a>
#+html: <a href="https://elpa.nongnu.org/nongnu/elfeed.html"><img alt="NonGNU ELPA" src="https://elpa.nongnu.org/nongnu/elfeed.svg"/></a>
#+html: <a href="https://elpa.nongnu.org/nongnu-devel/elfeed.html"><img alt="NonGNU-devel ELPA" src="https://elpa.nongnu.org/nongnu-devel/elfeed.svg"/></a>
#+html: <a href="https://releases.melpa.org/#/elfeed"><img alt="MELPA Releases" src="https://releases.melpa.org/packages/elfeed-badge.svg"/></a>
#+html: <a href="https://snapshots.melpa.org/#/elfeed"><img alt="MELPA Snapshots" src="https://snapshots.melpa.org/packages/elfeed-badge.svg"/></a>

Elfeed is an extensible web feed reader for Emacs, supporting Atom, RSS and JSON
Feed. It requires Emacs 29.1 and is available for download from [[https://elpa.nongnu.org/nongnu/elfeed.html][ELPA]] or [[https://releases.melpa.org/#/elfeed][MELPA]].
The Elfeed UI was inspired by [[https://notmuchmail.org/][notmuch]]. The database format is stable and is
...
We recommend that you install Elfeed via =M-x package-install=. You may want to
define a a global binding for the main entry point command =elfeed=.

#+begin_src emacs-lisp
(keymap-global-set "C-x w" #'elfeed)
#+end_src

Running the interactive function =elfeed= will pop up the =*elfeed-search*= buffer,
...
initiate an update with =M-x elfeed-update= (or press =G= in the Elfeed buffer).
...
Another option for providing a feed list is with an OPML file. Running =M-x elfeed-load-opml=
will fill =elfeed-feeds= with feeds listed in an OPML file. When
...
The last example assumes you've tagged posts with =youtube=. You probably want to
...
By default, entries tagged =unread= will have bolded titles in the =*elfeed-search*=
...
Elfeed includes a tree view, which gives an overview over all feeds and tags.
Invoke =M-x elfeed-tree= to open it. The feeds are visualized as a tree using the
feed auto tags. The first auto tag is the root node, the second tag comes below,
and so on. Feeds with the same auto tags are grouped together. Since
=outline-minor-mode= is enabled in the buffer you can use =TAB= and =S-TAB= to fold
like in Org-mode buffers. Use =RET= or click on tags or feeds to show the
...
Filters can be saved and restored using Emacs' built-in [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Bookmarks.html][bookmarks feature]]. While
in the search buffer, use =M-x bookmark-set= to save the current filter, and
=M-x bookmark-jump= to restore a saved filter. Emacs automatically persists
...
This link can be inserted into an Org-mode document. If the link is opened, the
search or entry will be shown in Elfeed.
...
- =link= : Org-mode link to this search, also available with =%a=, =%A=, =%l= and =%L=
- =description= : The search filter

List of available keywords, when link is stored from an Elfeed entry:

- =type= : Type of Org-mode link
- =link= : Org-mode link to this entry, also available with =%a=, =%A=, =%l= and =%L=
- =title= : Feed entry title
- =description= : Feed entry description, same as title
- =external-link= : Feed entry external link
- =date= : Date time of the feed entry publication, in full ISO 8601 format
- =date-timestamp= : Date time of the feed entry publication, in Org-mode active
  timestamp format
...
The =elfeed-feeds= variable allows you to set options per feed as a plist in
addition to the auto tags which come after the options. The options can be
accessed like the usual metadata via =(elfeed-meta feed :key)=. Right now Elfeed
supports these special keys which you may want to override in the =elfeed-feeds=
list.

- =:no-update= : Prevent a feed from updating when invoking =elfeed-update=.
- =:fetch-link= : Automatically fetch the content of the link and display it in
  the =elfeed-show= buffer.
...
You can define custom properties for your own purposes but you may consider
namespacing the keys with a prefix to avoid collisions.

** Metadata Plist

All feed and entry objects have plist where you can store your own arbitrary,
[[https://nullprogram.com/blog/2013/12/30/][readable values]]. These values are automatically persisted in the database. This
...
properties to avoid collisions (e.g. =:xyz/rating=), or simply not using keywords
as keys. Elfeed will always use keywords without a slash.
...
Emacs compiled with =libxml= bindings, which provides the necessary HTML parser.
...
- =c=: save url at point
- =f=: fetch link and display it in buffer (see also the =:fetch-link= feed option)
...
a [[https://github.com/emacs-elfeed/elfeed-web][separate repository]]. It's a single-page web application that follows the
...
There exists the [[https://github.com/areina/elfeed-cljsrn][elfeed-cljsrn]] Android application to access Elfeed via its web
API.
...
downloads with =url-retrieve=, not to mention the [[https://msdn.microsoft.com/en-us/library/kdfaxaay%28vs.71%29.aspx][tiny, hard-coded, 512 open
...
content database into a single compressed file. We recommend to execute
=elfeed-db-compact= from time to time manually.
...
Elfeed is a mature and maintained Emacs package, which should integrate well
into your Emacs setup. It is to the point where it can serve 100% of your web
feed needs, even if you use hundreds of feeds. We still have to decide on a
future roadmap, but our goal is to preserve the overall character and behavior
of Elfeed.
...
packages provide significant additional functionality and have more than 5K
downloads on [[https://melpa.org/#/?q=elfeed&sort=downloads&asc=false][MELPA]], which hints at a certain maturity. Note that no guarantees
for compatibility are made.

- [[https://github.com/fasheng/elfeed-protocol][elfeed-protocol]]: Support for many fetching methods.
- [[https://github.com/sp1ff/elfeed-score][elfeed-score]]: Gnus-like scoring.
- [[https://github.com/karthink/elfeed-tube][elfeed-tube]]: Enhancements for Youtube feeds.
- [[https://github.com/emacs-elfeed/elfeed-web][elfeed-web]]: Web interface to Elfeed.

Many more extensions can be found on MELPA - here is an non-exhaustive list:

- [[https://melpa.org/#/?q=elfeed&sort=downloads&asc=false][Elfeed-related packages on MELPA]]
- [[https://github.com/rtrppl/cuckoo-search][cuckoo-search]]
- [[https://github.com/benthamite/elfeed-ai][elfeed-ai]]
- [[https://github.com/paulelms/elfeed-autotag][elfeed-autotag]]
- [[https://github.com/rnadler/elfeed-curate][elfeed-curate]]
...
- [[https://github.com/fritzgrabo/elfeed-summarize][elfeed-summarize]]
- [[https://github.com/SqrtMinusOne/elfeed-summary][elfeed-summary]]
- [[https://github.com/zabe40/elfeed-time][elfeed-time]]

** Blog posts

- [[https://nullprogram.com/blog/2013/09/04/][Introducing Elfeed, an Emacs Web Feed Reader]].
- [[https://nullprogram.com/blog/2013/11/26/][Tips and Tricks]]
- [[https://pragmaticemacs.wordpress.com/emacs/read-your-rss-feeds-in-emacs-with-elfeed/][Read your RSS feeds in Emacs with Elfeed]]
- [[https://kitchingroup.cheme.cmu.edu/blog/2017/01/05/Scoring-elfeed-articles/][Scoring Elfeed articles]]
- [[https://www.youtube.com/watch?v=pOFqzK1Ymr4][Using Emacs 29]], [[https://www.youtube.com/watch?v=tjnK1rkO7RU][30]], [[https://www.youtube.com/watch?v=5zuSUbAHH8c][31]]
- [[https://babbagefiles.blogspot.com/2017/03/take-elfeed-everywhere-mobile-rss.html][Take Elfeed everywhere: Mobile rss reading Emacs-style (for free/cheap)]]
- [[https://noonker.github.io/posts/2020-04-22-elfeed/][Elfeed Rules!]] ([[https://old.reddit.com/r/emacs/comments/g6oowz/elfeed_rules/][reddit]])
- [[https://codingquark.com/emacs/2020/04/19/elfeed-protocol-ttrss.html][Elfeed with Tiny Tiny RSS]] ([[https://news.ycombinator.com/item?id=22915200][hn]])
- [[https://xenodium.com/open-emacs-elfeed-links-in-background/][Open Emacs elfeed links in the background]]
- [[https://web.archive.org/web/20241126185125/https://cestlaz.github.io/post/using-emacs-72-customizing-elfeed/][Using Emacs 72]]
- [[https://karthinks.com/blog/lazy-elfeed/][Lazy Elfeed]]
- [[https://medium.com/emacs/using-elfeed-to-view-videos-6dfc798e51e6][Using Elfeed to View Videos]]
- [[https://protesilaos.com/codelog/2020-09-11-emacs-elfeed-bongo/][Manage podcasts in Emacs with Elfeed and Bongo]]
- [[https://nullprogram.com/tags/elfeed/][Nullprogram Elfeed category]]
- [[https://pragmaticemacs.wordpress.com/category/elfeed/][Pragmaticemacs Elfeed category]]
