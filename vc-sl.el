...
;; Keywords: tools, vc
;; URL: https://github.com/swithinchan/vc-sl/
...
;; complements the separate `sl' package rather than replacing it: the
;; Sapling-specific stack/smartlog interface lives in `sl', while the
;; generic VC commands (`C-x v =', `C-x v d', `C-x v l', ...) are
;; served by this backend.
;;
;; To enable it, install the `sl' package, then add Sapling to
;; `vc-handled-backends':
;;
