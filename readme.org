...
Then obtain an access from your profile settings on your Forgejo instance, and either set =fj-token= to it (unencrypted option) or save it as an entry in your auth-sources file (encrypted option, =~/.authinfo.gpg= or similar; see variable =auth-sources=. If you don't want to use the encrypted option, you can also set =fj-token-use-auth-source= to nil.

Once you have done so, try =fj-list-own-repos= to view your repos, or =fj-list-issues= to view issues of the current repo, or with completing-read of a repo is no repo is found. If you are already viewing a repo and want to force the prompt to choose another, call =fj-list-issues= with a prefix arg (=C-u=). You can also call =fj-list-own-issues= to view a listing of all issues across your repos.
...
|           | fj-list-own-issues               | List issues in repos owned by `fj-user'.                            |
...
|           | fj-notifications-reload          | Reload current notifications view.                                  |
