...
To get going, first set =fj-host=,  =fj-user=, and =fj-token=. The token you can obtain from your profile settings.

* repositories

Current repository detection should work partially automatically. If you find you are still being asked for the repo when calling a command from a repo with a forgejo remote, check to make sure the local root directory matches the name of the forgejo repo!
...
|         | fj-repo-tl-list-user-repos    | View a tabulated list of current user from tabulated repos listing. |
