...
- list editing: `M-left` / `M-right` also nest the list item at point deeper or shallower, and `M-RET` inserts a sibling item (incrementing the number for explicitly-numbered lists)
...
- `outline-minor-mode` enabled out of the box for folding (one-line title style only)
