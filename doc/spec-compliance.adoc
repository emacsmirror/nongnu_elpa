...
. [x] *Add the `icon:name[]` inline macro* to the recognised macro set.
  Done.
. [ ] *Verify* CSV/DSV table formats and the literal-monospace `` `+text+` ``
  enclosure, and add handling where it is missing. Counters are done -
  `{counter:name}`, `{counter2:name}`, and `{set:name:value}` now highlight as
  attribute references (and character-replacement refs already did).
