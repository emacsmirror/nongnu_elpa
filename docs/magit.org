...
#+texinfo: @noindent
This manual is for Magit version 4.7.1.
...
Magit also provides context menus and other mouse commands, see [[*Mouse
Support]].
...
command ~magit-describe-section~ can be used to see which hook (if any)
...
- Key: C-c g (magit-dispatch) ::
  ...
  ~C-c g~ would be a better binding, but we cannot use that by default,
  because that key sequence is reserved for the user.  See [[*Global
  Bindings]] to learn more default and recommended key bindings.
...
- User Option: magit-no-confirm ::
  ...
  - Various:
    ...
    - ~stash-apply-3way~ When a stash cannot be applied using ~git stash
      apply~, then Magit uses ~git apply~ instead, possibly using the
      ~--3way~ argument, which isn't always perfectly safe.  See also
      ~magit-stash-apply~.
...
want to set ~magit-show-process-buffer-hint~ to ~nil~.
...
- Command: magit-parent-status ::

  This command shows the status of the parent repository of the
  current Git repository.  If the current repository is a submodule,
  it shows the status buffer of its super-repository.  Otherwise it
  steps outside the worktree of the current repository, and if that
  takes us to another repository, that is considered that the parent
  repository.
...
- Key: D T (magit-diff-toggle-fontify-hunk) ::

  This command toggles hunk fontification on or off, or switches the
  fontification method.
...
- User Option: magit-diff-fontify-hunk ::
...
- User Option: magit-format-file-function ::
  ...
  face &optional status orig)~.  KIND is one of ~diff~, ~module~, ~stat~ and
  ~list~.
...
- Key: RET (magit-diff-visit-file) ::
  ...
  diff is about, use ~magit-diff-visit-worktree-file~, described next.
...
by pressing ~C-c M-g~.  When using the recommended bindings, this
command is instead bound to ~C-c f~.  Also see [[*Global Bindings]].
...
- Key: C-c f B q ::
  ...
  the instructions in [[*Global Bindings]].
...
- Command: magit-copy-diff-as-kill ::
  ...
  It can be used on staged and unstaged changes, and anywhere
  ...
  This is useful if you need to copy some diff elsewhere, e.g., to a
  mail client or browser text area.
...
- Variable: remote.NAME.followRemoteHEAD ::
  ...
  is invoked.  This variable is documented in [[man:git-config]]
...
The following suffix is disabled by default. See
[[info:transient#Enabling and Disabling Suffixes]] for how to enable it.

- Key: F U (magit-pull-into-upstream) ::

  This command updates the upstream of the current branch from its own
  upstream.

  If the upstream of the current branch is a local branch, and the
  upstream of that is a remote branch, then the local upstream is
  updated.  This makes it possible to update the upstream branch,
  without having to first check it out.
...
- User Option: magit-push-options ::

  This option lists completion choices that are offered when setting
  ~--push-option~ in the ~magit-push~ menu.

  Note that ~--push-option~ is disabled by default. See
  [[info:transient#Enabling and Disabling Suffixes]] for how to enable it.

  Different forges support different push options.  Links to upstream
  documentation:

  - https://docs.gitlab.com/topics/git/commit/#push-options

  There should be similar documentation for other forges, but I could
  not find the respective summary pages.  If you find them, please let
  me know.
...
- Command: magit-show-commit-removing-file ::

  This command shows the commit that removed FILE.
...
The synchronous functions return the exit code, while the asynchronous
functions return the process object.
...
*** The diff/log is filtered to one file; how do I show all changes.

The diff arguments used in the current buffer can be changed using
the ~magit-diff-refresh~ menu on ~D~.

The log arguments used in the current buffer can be changed using
the ~magit-log-refresh~ menu on ~L~.
...
#+begin_quote
Copyright (C) 2015-{{{year}}} Jonas Bernoulli <emacs.magit@jonas.bernoulli.dev>
